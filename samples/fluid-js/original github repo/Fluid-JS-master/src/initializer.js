...
export function activator(canvas, webGL, colorFormat, PROGRAMS, pointers) {
    ...
    function splat(x, y, dx, dy, color, radius) {
        ...
        webGL.uniform1f(PROGRAMS.splatProgram.uniforms.radius, splatRadius / 100.0);
        blit(velocity.write.fbo);
        velocity.swap();
    ...
    function multipleSplats(amount) {
        // CUSTOM: Fekete folt (eredetileg: fehér 255,255,255)
        // Fekete háttéren láthatatlan lesz
        let color = {
            r: 0,
            g: 0,
            b: 0
    ...
    // CUSTOM: Export splat() function for external API access (createSplat() method)
    return {
        splat: splat,
        canvas: canvas
    };
